...
import { GITHUB_CLIENT_ID as gitHubClientId } from '@env'
...
export default function App() {
  ...
  async function handleGhOAuthResponse(response) {
    if (response?.type === 'success') {
      const { code } = response.params

      const token = await api
        .post('/register/gh', { code })
        .then(({ data }) => data.token)

      await SecureStore.setItemAsync('token', token)

      router.push('/memories/register')
    }
  }
  useEffect(() => {
    handleGhOAuthResponse(response)
  }, [response])
